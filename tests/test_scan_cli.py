...
def test_scan_verbose(sample_project: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(main, ["scan", str(sample_project), "--verbose"])
    assert result.exit_code == 0
...
def test_scan_with_output(sample_project: Path, tmp_path: Path) -> None:
    ...
    result = runner.invoke(main, ["scan", str(sample_project), "-o", str(output)])
    assert result.exit_code == 0
...
def test_scan_verbose_with_output(sample_project: Path, tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        main,
        ["scan", str(sample_project), "-o", str(output), "--verbose"],
    )
    assert result.exit_code == 0
